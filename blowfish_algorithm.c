...
void encrypt_file(const char* input_file, const char* output_file, const char* key) {
    ...
    uint8_t* output_buffer = (uint8_t*)malloc(file_size);
    ...
    size_t bytes_read = fread(input_buffer, 1, file_size, input);
    ...
    uint32_t* input_buffer_ptr = (uint32_t*)input_buffer;
    uint32_t* output_buffer_ptr = (uint32_t*)output_buffer;

    for (size_t i = 0; i < file_size / 4; i += 2) {
        Blowfish_Encrypt(&ctx, &input_buffer_ptr[i], &input_buffer_ptr[i + 1]);
        output_buffer_ptr[i] = input_buffer_ptr[i];
        output_buffer_ptr[i + 1] = input_buffer_ptr[i + 1];
    }

    fwrite(output_buffer, 1, file_size, output);
...
void decrypt_file(const char* input_file, const char* output_file, const char* key) {
    ...
    for (size_t i = 0; i < file_size / 4; i += 2) {
        Blowfish_Decrypt(&ctx, &input_buffer_ptr[i], &input_buffer_ptr[i + 1]);
        output_buffer_ptr[i] = input_buffer_ptr[i];
        output_buffer_ptr[i + 1] = input_buffer_ptr[i + 1];
    }
